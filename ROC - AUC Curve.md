...
## False Positive Report ( FPR) 
 ...
 ## NOW HOW ROC is constructed:
    1. If the modelprediction for 5 inputs , then output probablity for each value will be
    ...
    4. So for the above five samples model give 5 predicted values , based on the model learning , now we will use threshold to make it as 0/1 if 
    ...
    7. Predicted labels , model predicted output based on threshold  value
...
5.     Area of two consecutive points is (x1, y1) and (x2, y2) => (x2-x1) * (y1 +  y2) / 2 [ Area of trapeziods , these are increasing step by step so , height will be x2-x1 , and paralel sides y2,y1 ,
   Area Of Trapezoids  = 1/2 (a+b) * h
   here
   - a,b= Paraller side   ===> that is y1 and y2.
   - h = height of hrixonata line , point move from x1 to x2 hence => h = x2-x1
   - Hence , Area = 1/2 (y2 + y1) * (x2-x1).
7. So we find area from 0 to first point => AUC1 , first point to second point => AUC2 , second to third => AUC3
8. So total AUC = AUC1 + AUC2 + AUC 3
9. This how AUC is calculated from the plotted points.
10. ITs the result  of total area of the ROC curve of 0 to 1 probablity , hece it always comes in 0 to 1
11.  if AUC = 1 , then model  have 100% higher probablity than lower probabloty , if its 0.5 its randomly calculating , if its lesser than 0.5
12.  then  it predicting inversly.
13.  
